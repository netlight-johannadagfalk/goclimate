...
const ProjectSummary = ({ project }) => {
  ...
  useEffect(() => {
    ...
    const descriptionElement = document.getElementById("description"+project.id)
    const projectBoxElement = document.getElementById("projectBox"+project.id)
    setTempHeight(projectBoxElement?.clientHeight+3)
    if (titleElement?.scrollHeight + descriptionElement?.scrollHeight < 125) {
      setShortText(true)
    }
  }, []);
  ...
  return (
    <div className="t:w-1/3 pt-14 t:px-1 self-center t:self-start max-w-sm w-full" >
      <div id={"projectBox"+project.id} className="callout space-y-3 pt-0 p-4 text-left collapse min-h-1/4 w-full" style={{minHeight: +tempHeight+"px", minWidth: "100%"}}>
        ...
        >
          <h3 className="font-semibold" id={"title"+project.id}>{project.name}</h3>
          <p className="pt-2" id={"description"+project.id}>{project.short_description}</p>
        </div>
        { !shortText &&
          <div
